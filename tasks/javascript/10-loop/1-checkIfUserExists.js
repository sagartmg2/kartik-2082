...
function loginUserSolution1(email, password) {
  let matched = false;
  ...
  console.log(matched ? "login success" : "login failed");

  console.log("end");
}

function loginUserSolution2(inputEmail, inputPassword) {
  let matched = false;

  for (let index = 0; index < dbUsers.length; index++) {
    let { password, email } = dbUsers[index];
...
const loginUserSolution3 = (inputEmail, inputPassword) => {
  let matched = false;
...
const loginUser = (inputEmail, inputPassword) => {
  let matched = dbUsers.find(
    (el) => el.password == inputPassword && el.email == inputEmail
  );
  console.log(matched ? "login success" : "login failed");
};
...
// let type = typeof("132234")
// console.log(typeof("132234"));
// console.log(type);

const double1 = (number) => number * 2;
const double = (number) => number * 2;

console.log({ "dobule 2": double(2) });
console.log({ "dobule 3": double(3) });

let fruits = ["apple", "orange", "kiwi"];

let exists = fruits.find((el) => el == "banana");
console.log(exists);
